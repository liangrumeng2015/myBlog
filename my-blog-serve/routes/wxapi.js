 ...
 */
router.post('/getOpenId.do',(req,res)=>{
 ...
 * 生物认证接口
 */
router.post('/soter.do',(req,res)=>{
    let openid = req.body.openid;
    let json_string = req.body.json_string;
    let json_signature = req.body.json_signature;
    let access_token = req.body.access_token
    let url = 'http://api.weixin.qq.com/cgi-bin/soter/verify_signature?access_token='+access_token;
    var data = {
        openid,
        json_string,
        json_signature
    }
    request.post({url,form:data},(err,data)=>{
        if(!err){
            res.json({
                module:data,
                success:true
